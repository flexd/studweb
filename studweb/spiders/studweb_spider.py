...
class StudwebSpider(BaseSpider):
    ...
    start_urls = [
        "https://www.studweb.no/as/WebObjects/studentweb2.woa/wa/default?inst=HiOA"
    ...
    def after_login(self, response):
      ...
      innsyn_url = hxs.select('//*[@id="navig-menu"]/table/tr[5]/td[1]/a/@href').extract()[0]
      return Request('https://www.studweb.no%s' % innsyn_url, callback=self.after_innsyn)
    def after_innsyn(self, response):
      hxs = HtmlXPathSelector(response)
      resultater_url = hxs.select('//*[@id="navig-menu"]/table/tr[8]/td[1]/a/@href').extract()[0]
    ...
    def parse_results(self, response):
      ...
      hxs = HtmlXPathSelector(response)
      studiepoeng = hxs.select('/html/body/table/tr[3]/td[2]/table/tr[last()]/td[3]/text()').extract()[0]
      ...
      if forrige_studiepoeng is None:
        ...
        if studiepoeng > float(forrige_studiepoeng):
          nma.push('Karakterer', 'Noe?', 'Antall studiepoeng har gått opp!')
          r.set('forrige_studiepoeng', studiepoeng)
